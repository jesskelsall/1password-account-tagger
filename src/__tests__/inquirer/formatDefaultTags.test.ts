...
test('returns an array of tag values', async () => {
...
test('returns no tag values when no tags are selected', async () => {
...
test('returns some tag values when some tags are selected', async () => {
  const result = formatDefaultTags(preparedTags)
...
test('returns all tag values when all tags are selected', async () => {
